...
const _searchItems = <Type>(items: SearchRecord<Type>[], search: string, args: Readonly<SearchArgs>): SearchRecord<Type>[] => {
  ...
  for (const item of items) {
      ...
      if (contains / search.length < (args.containsCharsTolerance as any))
          continue;
      ...
      for (let t = 0; t < item.tokens.length; t++) {
          ...
          if ((subCandidate === Infinity || edits < editsCandidate) && item.tokens[t].indexOf(search) !== -1) {
              ...
              if ((edits / Math.max(search.length, item.tokens[t].length)) <= (args.editsDistanceTolerance as any)) {
...
export const searchItems = <Type>(items: [string, Type][], search = '', args: SearchArgs = {}): Type[] => {
  ...
  if (args.hasOwnProperty('limitResults') && recordItems.length > (args.limitResults as any)) {
