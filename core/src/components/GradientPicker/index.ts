...
import { CurveSet, Curve, math } from '../../helpers/math';
import { _hsv2rgb, _rgb2hsv } from '../../helpers/color-value';

const CLASS_MULTIPLE_VALUES = PREFIX + 'multiple-values';
...
const CLASS_GRADIENT = PREFIX + 'gradient';

/**
 * {@link GradientPicker}构造函数的参数。
 */
export interface GradientPickerArgs extends ElementArgs {
    /**
     * 如果为true，选择器会在渐变发生变化时渲染渐变。默认为“true”
     */
    renderChanges?: boolean;
    /**
     * 一个包含4个整数的数组，其中包含选择器应该初始化为的RGBA值。
     */
    value?: number[];
    /**
     * 颜色通道数。默认为3。更改为4增加了更改alpha值的选项。
     */
    channels?: number;
}
...
class GradientPicker extends Element {
    ...
    protected _checkerboardPattern: CanvasPattern | null;
     ...
     * 创建一个新的渐变选择器
     *
     * @param args - 参数，扩展自Element参数，任何可设置的属性也可以通过构造函数设置。
     */
    constructor(args: Readonly<GradientPickerArgs> = {}) {
        ...
        (this.dom as any).appendChild(this._canvas.dom);
        this._canvas.parent = this;
        this._canvas.on('resize', () => {
            this._renderGradient();
        });
        const canvasElement = this._canvas.dom as HTMLCanvasElement;
        this._checkerboardPattern = this._createCheckerboardPattern(canvasElement.getContext('2d') as any);
        ...
        this.dom && this.dom.addEventListener('keydown', this._onKeyDown);
        this.dom && this.dom.addEventListener('focus', this._onFocus);
        this.dom && this.dom.addEventListener('blur', this._onBlur);
        ...
        this.dom && this.dom.appendChild(this._panel.dom as any);
        ...
        function numberField(label: string) {
            ...
            field.on('change', that.fieldChangeHandler);
            that._fields.appendChild(field.dom);
            return field;
        }
        ...
        this._fields.appendChild(that._hexField.dom);

        // hook up mouse handlers
        that._colorRect.dom.addEventListener('mousedown', this.downHandler);
        that._hueRect.dom.addEventListener('mousedown', this.downHandler);
        that._alphaRect.dom.addEventListener('mousedown', this.downHandler);
        ...
        // current state
        this.STATE = {
            curves: [],            // 保存所有的梯度曲线(3条或4条)
            keystore: [],          // 在编辑曲线
            anchors: [],           // 保存锚的时间
            hoveredAnchor: -1,     // 悬停锚的索引
            selectedAnchor: -1,    // 选定锚的指标
            selectedValue: [],     // 被拖动的值
            changing: false,       // UI正在改变
            draggingAnchor: false,
            typeMap: { }          // 从曲线类型下拉映射到发动机曲线枚举
        };
        ...
        this.on('changing', function (color) {
            that.colorSelectedAnchor(color, true);
        });
    ...
    destroy() {
        ...
        this.dom && this.dom.removeEventListener('keydown', this._onKeyDown);
    ...
    protected _openGradientPicker() {
        ...
        const rectField = this.dom && this.dom.getBoundingClientRect() as any;
    ...
    set value(value) {
        // TODO: 
        // 也许应该检查是否相等，但由于这个值几乎总是使用选择器设置，所以不值得这样做
        this._value = value;
    ...
    protected _generateHue(canvas: Canvas) {
        ...
        const ctx = canvasElement.getContext('2d') as any;
    ...
    protected _onHexChanged() {
        if (!this._changing) {
            const hex = (this._hexField.value as string).trim().toLowerCase();
            if (/^([0-9a-f]{2}){3,4}$/.test(hex)) {
                const rgb = [parseInt(hex.substring(0, 2), 16),
                    parseInt(hex.substring(2, 4), 16),
                    parseInt(hex.substring(4, 6), 16)];
                (this.hsva as any) = _rgb2hsv(rgb).concat([this.hsva[3]]);
                this.colorSelectedAnchor(this.color);
            }
        }
    }
    ...
    set hsva(hsva) {
        ...
        const e = this._colorRect.dom as any;
    ...
    set color(clr) {
        ...
        if (hsva[0] === 0 && hsva[1] === 0 && this._hsva[0] !== -1) {
            // 如果传入的RGB是一个灰色阴影(没有色调)，使用当前的活动色调代替。
            hsva[0] = this._hsva[0];
        }
        this.hsva = hsva;
    }
    ...
    set editAlpha(editAlpha) {
        const that = this as any;
        if (editAlpha) {
            that._alphaRect.dom.style.display = 'inline';
            that._alphaHandle.style.display = 'block';
            that._aField.dom.style.display = 'inline-block';
        } else {
            that._alphaRect.dom.style.display = 'none';
            that._alphaHandle.style.display = 'none';
            that._aField.dom.style.display = 'none';
        }
    }
