...
const CLASS_ROOT = PREFIX + 'color-input';

/**
 * {@link ColorPicker}构造函数的参数
 */
export interface ColorPickerArgs extends ElementArgs, IBindableArgs {
    /**
     * 一个包含4个整数的数组，其中包含选择器应该初始化为的RGBA值。默认为'[0,0,255,1]'。
     */
    value?: number[];
    /**
...
class ColorPicker extends Element implements IBindable {
    ...
    protected _historyPostfix: string|undefined|null = null;
    ...
    protected _evtColorPick: EventHandle|null = null;

    protected _evtColorToPicker: EventHandle|null = null;

    protected _evtColorPickStart: EventHandle|null = null;

    protected _evtColorPickEnd: EventHandle|null = null;
    ...
    constructor(args: Readonly<ColorPickerArgs> = {}) {
        ...
        (this.dom as any).appendChild(this._domColor);
        ...
        (this.dom as any).appendChild(this._overlay.dom);
        ...
        this._pickRect.addEventListener('mousedown', function (evt: MouseEvent) {
            that._pickRectMouseMove(evt);

            window.addEventListener('mousemove', that._pickRectMouseMove, false);
            window.addEventListener('mouseup', that._pickRectMouseUp, false);

            evt.stopPropagation();
            evt.preventDefault();
            that._dragging = true;
            that.emit('picker:color:start');
        }.bind(this));
        ...
        this._pickHue.addEventListener('mousedown', function (evt: MouseEvent) {
            that._pickHueMouseMove(evt);

            window.addEventListener('mousemove', that._pickHueMouseMove, false);
            window.addEventListener('mouseup', that._pickHueMouseUp, false);
        ...
        this._pickOpacity.addEventListener('mousedown', function (evt: MouseEvent) {
            that._pickOpacityMouseMove(evt);

            window.addEventListener('mousemove', that._pickOpacityMouseMove, false);
            window.addEventListener('mouseup', that._pickOpacityMouseUp, false);
        ...
        this._overlay.on('hide', () => {
            (that._evtColorPick as any).unbind();
            this._evtColorPick = null;

            (that._evtColorToPicker as any).unbind();
            this._evtColorToPicker = null;

            (that._evtColorPickStart as any).unbind();
            this._evtColorPickStart = null;

            (that._evtColorPickEnd as any).unbind();
        ...
        this._panelFields.appendChild(fieldR.dom as any);
        ...
        this._panelFields.appendChild(fieldG.dom as any);
        ...
        this._panelFields.appendChild(fieldB.dom as any);
        ...
        this._panelFields.appendChild(fieldA.dom as any);
        ...
        this._panelFields.appendChild(this._fieldHex.dom as any);
    }
    ...
    protected _setPickerColor(color: number[]) {
        ...
        if (this._channelsNumber >= 3) {
            ...
            (this._colorHSV[0] as any) = hsv[0];
            (this._colorHSV[1] as any) = hsv[1];
            (this._colorHSV[2] as any) = hsv[2];
        }
    ...
    protected _openColorPicker() {
        ...
        this._evtColorPickStart = this.on('picker:color:start', () => {
            if (this.binding) {
                ...
                (this._binding as any).historyPostfix = `(${Date.now()})`;
        ...
        this._evtColorPickEnd = this.on('picker:color:end', () => {
            if (this.binding) {
                ...
                (this.binding as any).historyPostfix = this._historyPostfix;
            }
        });

        // position picker
        const rectPicker = (this._overlay.dom as any).getBoundingClientRect();
        const rectElement = (this.dom as any).getBoundingClientRect();
    ...
    protected _callPicker(color: number[]) {
        ...
        // focus on hex field
        (this._fieldHex.dom as any).focus();
    ...
    protected _onChangeHex() {
        ...
        const hex = (this._fieldHex.value as string).trim().toLowerCase();
